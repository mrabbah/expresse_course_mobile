define(['jquery', 'commun', 'jqm', 'iscroll', 'iscrollview', 'layer_xy_hack'], function ($, commun) {  

    var chat = {
        init : function() {
            console.log('calling init chat.js');
            initF();
        }
    };
    return chat;
...
function chargerMessages(offset) {        
    ...
    commun.askServer('getMessagesTache',{idtache : idtache, offset : offset}, messagesCharges,true);
};
function messagesCharges(data) {
    console.log(commun.syntaxHighlight(data));
...
function initactionschat() {    
    ...
    $("#envoyermessagetext").click(function() {
        ...
        if (message.length == 0) {
            ...
            commun.askServer('envoyerMessage',{idtache : idtache, message: message}, messageEnvoyeSuccess,true);            
        }
    });
}
;
function messageEnvoyeSuccess(data) {
